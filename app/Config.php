 ...
 * 
 * CRITICAL: This class is now strictly DB-independent during the routing phase.
 * It allows the installer to boot even when database credentials are missing.
 */
final class Config
{
    ...
    public static function registerRoutes(App $app): void
    {
        ...
        // 2. Identify if we are in install mode to skip other routes if necessary
        // However, we just need to ensure other routes don't call the container yet.
    ...
    private static function registerApiRoutes(App $app, string $typePattern): void
    {
        $app->group('/api/v1', function (RouteCollectorProxy $group) use ($typePattern): void {
            ...
            $group->post('/auth/register', [AuthController::class, 'register'])->add(RateLimitKeyedMiddleware::class);
            $group->post('/auth/login', [AuthController::class, 'login'])->add(RateLimitKeyedMiddleware::class);
            $group->post('/auth/refresh', [AuthController::class, 'refresh'])->add(RateLimitMiddleware::class);
            ...
            $group->group('', function (RouteCollectorProxy $secure) use ($typePattern): void {
                ...
                $secure->post('/chapter/{chapterId:[a-z0-9]{6}}/comment', [CommentController::class, 'create'])->add(RestrictedActionMiddleware::class);
                $secure->post('/comments/{commentId:[0-9]+}/vote', [CommentController::class, 'vote'])->add(RestrictedActionMiddleware::class);
                ...
                $secure->post('/blogs', [BlogController::class, 'create'])->add(RestrictedActionMiddleware::class);
                $secure->post('/blogs/image', [BlogController::class, 'uploadImage']);
                $secure->post('/blogs/{slug}/vote', [BlogController::class, 'vote'])->add(RestrictedActionMiddleware::class);
                $secure->post('/blogs/{slug}/comments', [CommentController::class, 'createBlog'])->add(RestrictedActionMiddleware::class);
                $secure->post('/blogs/{slug}/comments/{commentId:[0-9]+}/vote', [CommentController::class, 'voteBlog'])->add(RestrictedActionMiddleware::class);
    ...
    private static function registerAdminRoutes(App $app, string $typePattern): void
    {
        ...
        })->add(RateLimitMiddleware::class)->add(CsrfMiddleware::class)->add(AuthMiddleware::class);
    }
}

...
final class Config
{
    ...
    public static function getSettings(): array
    {
        ...
        self::$cachedSettings = [
            "app" => [
                ...
                "env" => strtolower(
                    (string) self::env("APP_ENV", "production"),
                ),
                ...
                "session_lifetime_seconds" => (int) self::env(
                    "SESSION_LIFETIME",
                    7200,
                ),
                "refresh_token_days" => (int) self::env(
                    "REFRESH_TOKEN_DAYS",
                    30,
                ),
    ...
    public static function getSystemConfig(): array
    {
        return [
            ...
            "site_abbreviation" => (string) self::env(
                "SITE_ABBREVIATION",
                "NMR",
            ),
            "site_logo" => (string) self::env(
                "SITE_LOGO",
                "/assets/img/logo.svg",
            ),
            "site_description" => (string) self::env(
                "SITE_DESCRIPTION",
                "Read manga and novels for free.",
            ),
            "enforce_https" => (bool) self::env("ENFORCE_HTTPS", false),
            "site_address" => (string) self::env(
                "SITE_ADDRESS",
                "https://example.com",
            ),
            "default_language" => (string) self::env("DEFAULT_LANGUAGE", "tr"),
            "default_theme" => (string) self::env("DEFAULT_THEME", "dark"),
            "default_profile_image" => (string) self::env(
                "DEFAULT_PROFILE_IMAGE",
                "/assets/img/default-profile.png",
            ),
            "default_content_cover_image" => (string) self::env(
                "DEFAULT_CONTENT_COVER_IMAGE",
                "/assets/img/covers/one-piece.jpg",
            ),
            "integrations" => [
                "google_analytics_id" => (string) self::env(
                    "GOOGLE_ANALYTICS_ID",
                    "",
                ),
                "google_recaptcha_site_key" => (string) self::env(
                    "GOOGLE_RECAPTCHA_SITE_KEY",
                    "",
                ),
                "google_recaptcha_secret_key" => (string) self::env(
                    "GOOGLE_RECAPTCHA_SECRET_KEY",
                    "",
                ),
                "cloudflare_turnstile_site_key" => (string) self::env(
                    "CLOUDFLARE_TURNSTILE_SITE_KEY",
                    "",
                ),
                "cloudflare_turnstile_secret_key" => (string) self::env(
    ...
    private static function registerWebRoutes(
        ...
        $addWebRoutes = function (
            ...
            $group->get("/{type:" . $typePattern . "}", [
                WebController::class,
                "listing",
            ]);
            $group->get(
                "/{type:" . $typePattern . "}/{slug}/chapter/{chapterNumber}",
                [WebController::class, "chapter"],
            );
            $group->get("/{type:" . $typePattern . "}/{slug}", [
                WebController::class,
                "content",
            ]);
            $group->get("/login", [WebController::class, "login"]);
            $group->get("/profile", [WebController::class, "profile"]);
            $group->get("/profile/{person:[A-Za-z0-9_]+}", [
                WebController::class,
                "profile",
            ]);

            $group->get("/admin", [WebController::class, "adminDashboard"]);
            $group->get("/admin/content", [
                WebController::class,
                "adminContent",
            ]);
            $group->get("/admin/blogs", [WebController::class, "adminBlogs"]);
            $group->get("/admin/comments", [
                WebController::class,
                "adminComments",
            ]);
        ...
        $app->group("/{lang:tr|en}", function (RouteCollectorProxy $group) use (
            $addWebRoutes,
        ): void {
    ...
    private static function registerApiRoutes(
        ...
        $app->group("/api/v1", function (RouteCollectorProxy $group) use (
            ...
            $group->get("/content/type/{type:" . $typePattern . "}", [
                SeriesController::class,
                "byType",
            ]);
            $group->get("/content/{type:" . $typePattern . "}/{slug}", [
                SeriesController::class,
                "contentByType",
            ]);
            $group->get(
                "/content/{type:" . $typePattern . "}/{slug}/chapters",
                [SeriesController::class, "chaptersByType"],
            );
            $group->get("/genre/{slug}", [SeriesController::class, "genre"]);
            $group->get("/tag/{slug}", [SeriesController::class, "tag"]);
            $group->get("/latest-chapters", [
                SeriesController::class,
                "latestChapters",
            ]);

            $group->get("/series_genres", [
                SeriesController::class,
                "series_genres",
            ]);
            $group->get("/series_tags", [
                SeriesController::class,
                "series_tags",
            ]);

            $group->get("/content/{type:" . $typePattern . "}/chapters", [
                SeriesController::class,
                "latestChaptersByType",
            ]);
            $group->get("/profile/{person:[A-Za-z0-9_]+}", [
            ...
            $group
                ->get("/blogs/{slug}", [BlogController::class, "show"])
                ->add(new AuthMiddleware(true, $authorization));
            $group->get(
                "/content/{type:" .
                    $typePattern .
                    "}/{slug}/chapter/{chapterNumber}",
                [ChapterController::class, "showByContent"],
            );
            $group->get("/search", [SeriesController::class, "search"]);
            $group->get("/search/suggest", [
                SeriesController::class,
                "suggest",
            ]);
            $group->get("/i18n/{lang:[a-z]{2}}", [
                WebController::class,
                "i18nJson",
            ]);
            $group->post("/log/error", [WebController::class, "logError"]);
            $group
                ->post("/user/activity", [ActivityController::class, "track"])
                ->add(new AuthMiddleware(true, $authorization));
            $group->get("/chapter/{chapterNumber}", [
                ChapterController::class,
                "show",
            ]);
            $group->get("/chapter/{chapterId:[a-z0-9]{6}}/comments", [
                CommentController::class,
                "list",
            ]);
            $group->get(
                "/content/{type:" . $typePattern . "}/{slug}/comments",
                [CommentController::class, "listSeries"],
            );
            $group->get("/blogs/{slug}/comments", [
                CommentController::class,
                "listBlog",
            ]);
                ...
                ->add(
                    new RateLimitKeyedMiddleware(
                        ...
                        fn($req) => "email:" .
                            strtolower(
                                trim(
                                    (string) ($req->getParsedBody()["email"] ??
                                        ""),
                                ),
                            ),
                    ),
            ...
            $group->map(["GET", "POST"], "/auth/logout", [
                AuthController::class,
                "logout",
            ]);

            $group
                ->group("", function (RouteCollectorProxy $secure) use (
                    $typePattern,
                    $users,
                ): void {
                    $secure->post(
                        "/content/{type:" . $typePattern . "}/{slug}/follow",
                        [SeriesController::class, "followByType"],
                    );
                    $secure->delete(
                        "/content/{type:" . $typePattern . "}/{slug}/follow",
                        [SeriesController::class, "unfollowByType"],
                    );
                    $secure->post(
                        "/content/{type:" . $typePattern . "}/{slug}/rate",
                        [RatingController::class, "rateByType"],
                    );
                    $secure
                        ->post(
                            "/content/{type:" .
                                $typePattern .
                                "}/{slug}/comment",
                            [CommentController::class, "createSeries"],
                        )
                        ->add(
                            new RestrictedActionMiddleware(
                                $users,
                                "commenting",
                            ),
                        );
                    $secure
                        ->post("/chapter/{chapterId:[a-z0-9]{6}}/comment", [
                            CommentController::class,
                            "create",
                        ])
                        ->add(
                    ...
                    $secure
                        ->post("/comments/{commentId:[0-9]+}/vote", [
                            CommentController::class,
                            "vote",
                        ])
                        ->add(new RestrictedActionMiddleware($users, "voting"));
                    $secure->post("/user/profile", [
                        UserController::class,
                        "updateProfile",
                    ]);
                    ...
                    $secure->get("/user/history", [
                        UserController::class,
                        "history",
                    ]);
                    $secure->get("/user/preferences", [
                        UserController::class,
                        "preferences",
                    ]);
                    $secure->put("/user/preferences", [
                        UserController::class,
                        "updatePreferences",
                    ]);

                    $secure->get("/user/follows", [
                        SeriesController::class,
                        "followed",
                    ]);
                    $secure->get("/user/blogs", [
                    ...
                    $secure
                        ->post(
                            "/blogs/{slug}/comments/{commentId:[0-9]+}/vote",
                            [CommentController::class, "voteBlog"],
                        )
                        ->add(new RestrictedActionMiddleware($users, "voting"));
                    $secure->get("/auth/sessions", [
                        AuthController::class,
                        "sessions",
                    ]);
                    $secure->delete("/auth/sessions/{sessionKey:[a-z0-9]+}", [
                        AuthController::class,
                        "revokeSession",
                    ]);
                    $secure->get("/user/notifications", [
                        UserController::class,
                        "notifications",
                    ]);
                    $secure->post("/user/notifications/read", [
                        UserController::class,
                        "markNotificationsRead",
                    ]);
                    $secure->get("/user/follows/users", [
                        UserController::class,
                        "followedUsers",
                    ]);
                    $secure->post("/user/follows/{person:[A-Za-z0-9_]+}", [
                        UserController::class,
                        "follow",
                    ]);
                    $secure->delete("/user/follows/{person:[A-Za-z0-9_]+}", [
                        UserController::class,
                        "unfollow",
                    ]);
                })
    ...
    private static function registerAdminRoutes(
        ...
        $perm = static fn(
            array $p,
        ): PermissionMiddleware => new PermissionMiddleware($p);

        $app->group("/api/v1/admin", function (RouteCollectorProxy $group) use (
            $typePattern,
            $perm,
            $cache,
        ): void {
            $group
                ->get("/overview", [AdminConsoleController::class, "overview"])
                ->add($perm(["admin.panel.access"]));
            $group
                ->get("/series", [AdminConsoleController::class, "series"])
                ->add($perm(["admin.panel.access"]));
            $group
                ->get("/contents", [AdminConsoleController::class, "series"])
                ->add($perm(["admin.panel.access"]));
            $group
                ->get("/genres", [AdminConsoleController::class, "listGenres"])
                ->add($perm(["admin.panel.access"]));
            $group
                ->get("/tags", [AdminConsoleController::class, "listTags"])
                ->add($perm(["admin.panel.access"]));
            $group
            ...
            $group
                ...
                ->add($perm(["admin.panel.access"]));
            $group
                ->get("/blogs/pending", [BlogController::class, "pending"])
                ->add($perm(["admin.panel.access"]));
            $group
                ->get("/comments", [AdminConsoleController::class, "comments"])
                ->add($perm(["admin.panel.access"]));
            $group
                ...
                ->add($perm(["admin.comment.delete"]));
            $group
                ->put("/users/{id}", [
                    AdminConsoleController::class,
                    "updateUser",
                ])
                ->add($perm(["admin.users.manage"]));
            $group
                ->get("/rbac/roles", [
                    AdminConsoleController::class,
                    "rbacRoles",
            ...
            $group
                ...
                ->add($perm(["admin.jobs.run"]));
            $group
                ->post("/maintenance/sitemap", [
                    AdminConsoleController::class,
                    "triggerSitemap",
                ])
                ->add($perm(["admin.jobs.run"]));
            $group
                ->post("/maintenance/warmup", [
                    AdminConsoleController::class,
                    "triggerCacheWarmup",
                ])
                ->add($perm(["admin.jobs.run"]));
            ...
            $group
                ...
                ->add($perm(["admin.metrics.view"]));
            $group
                ->get("/metrics", [MetricsController::class, "snapshot"])
                ->add($perm(["admin.metrics.view"]));
            $group
                ->get("/dashboard", [MetricsController::class, "snapshot"])
                ->add($perm(["admin.metrics.view"]));
            $group
            ...
            $group
                ->post("/content", [AdminController::class, "createContent"])
                ->add($perm(["admin.content.create"]));
            $group
                ->post("/upload-images", [
            ...
            $group
                ...
                ->add($perm(["admin.content.update"]));
            $group
                ->put("/contents/{id}/taxonomy", [
                    AdminConsoleController::class,
                    "updateTaxonomy",
                ])
                ->add($perm(["admin.content.update"]));
            $group
                ->post("/content/{type:" . $typePattern . "}/{slug}/chapters", [
                    AdminController::class,
                    "createChapter",
                ])
                ->add($perm(["admin.chapter.create"]));
            $group
                ->get("/content/{id}/chapters", [
                    AdminController::class,
                    "listChapters",
                ])
                ->add($perm(["admin.panel.access"]));

...
$(function () {
  ...
  /**
   * Injects modal HTML structures into the DOM if they don't exist.
   */
  const injectModals = () => {
    ...
    const allModals = `
      <div id="loginModal" class="modal-overlay">
        <div class="modal card">
          <div class="modal-header"><h3>🔐 ${NMR.__t('login')}</h3><button class="modal-close" onclick="closeModal()">&times;</button></div>
          <div class="modal-body">
            <form id="loginForm">
              <div class="form-group"><label class="form-label">${NMR.__t('email')}</label><input type="email" class="form-item" required></div>
              <div class="form-group"><label class="form-label">${NMR.__t('password')}</label><input type="password" class="form-item" required></div>
              <div class="form-group flex items-center gap-2 mb-2">
                <input type="checkbox" id="loginRemember" style="width:auto;cursor:pointer">
                <label for="loginRemember" style="cursor:pointer;font-size:0.85rem;user-select:none">${NMR.__t('remember_me')}</label>
              </div>
              <button type="submit" class="btn btn-primary w-100 mt-2">${NMR.__t('login')}</button>
            </form>
          </div>
          <div class="modal-footer"><button class="btn btn-sm btn-outline" onclick="openModal('registerModal')">${NMR.__t('create_account')}</button></div>
        </div>
      </div>
      <div id="registerModal" class="modal-overlay">
        <div class="modal card">
          <div class="modal-header"><h3>✨ ${NMR.__t('signup')}</h3><button class="modal-close" onclick="closeModal()">&times;</button></div>
          <div class="modal-body">
            <form id="registerForm">
              <div class="form-group"><label class="form-label">${NMR.__t('username')}</label><input type="text" class="form-item" required></div>
              <div class="form-group"><label class="form-label">${NMR.__t('email')}</label><input type="email" class="form-item" required></div>
              <div class="form-group"><label class="form-label">${NMR.__t('password')}</label><input type="password" class="form-item" required></div>
              <button type="submit" class="btn btn-primary w-100 mt-2">${NMR.__t('signup')}</button>
            </form>
          </div>
          <div class="modal-footer"><button class="btn btn-sm btn-outline" onclick="openModal('loginModal')">${NMR.__t('back_to_login')}</button></div>
        </div>
      </div>
      <div id="notifModal" class="modal-overlay">
        <div class="modal card">
          <div class="modal-header"><h3>🔔 ${NMR.__t('notifications')}</h3><button class="modal-close" onclick="closeModal()">&times;</button></div>
          <div class="modal-body p-0" style="max-height: 400px; overflow-y: auto;" id="notifModalList">
            <div class="p-4 text-center text-muted">${NMR.__t('loading')}</div>
          </div>
          <div class="modal-footer"><button class="btn btn-sm btn-outline w-100" id="markAllReadBtn">${NMR.__t('mark_all_read')}</button></div>
        </div>
      </div>
      <div id="readerSettingsModal" class="modal-overlay">
        <div class="modal card modal-wide">
          <div class="modal-header"><h3>⚙️ ${NMR.__t('reader_settings')}</h3><button class="modal-close" onclick="closeModal()">&times;</button></div>
          <div class="modal-body p-0"><div class="flex" style="min-height: 400px;">
            <div class="modal-sidebar border-r p-2" id="readerTabSidebar" style="width: 160px; background: var(--surface)">
              <button class="btn btn-sm btn-outline w-100 text-left mb-1 active" data-tab="layout">${NMR.__t('layout')}</button>
              <button class="btn btn-sm btn-outline w-100 text-left mb-1" data-tab="typography">${NMR.__t('typography')}</button>
              <button class="btn btn-sm btn-outline w-100 text-left mb-1" data-tab="appearance">${NMR.__t('appearance')}</button>
            </div>
            <div class="modal-content-area flex-grow p-4">
              <div id="tab-layout" class="settings-tab">
                <div class="form-group mb-3"><label class="text-xs">${NMR.__t('page_layout')}</label>
                  <div class="btn-group w-100" id="layoutButtonGroup">
                    <button class="btn btn-sm btn-outline" data-val="single">${NMR.__t('single_page')}</button>
                    <button class="btn btn-sm btn-outline" data-val="double">${NMR.__t('double_page')}</button>
                    <button class="btn btn-sm btn-outline" data-val="vertical">${NMR.__t('vertical_scroll')}</button>
                  </div>
                </div>
                <div class="form-group"><label class="text-xs">${NMR.__t('image_fit')}</label>
                  <select class="form-item" name="reader_image_fit"><option value="width">${NMR.__t('fit_width')}</option><option value="height">${NMR.__t('fit_height')}</option><option value="original">${NMR.__t('original_size')}</option></select>
                </div>
              </div>
              <div id="tab-typography" class="settings-tab hidden">
                <div class="form-group mb-3"><label class="text-xs">${NMR.__t('font_family')}</label>
                  <select class="form-item" name="reader_font_family"><option value="var(--font-sans)">Sans</option><option value="serif">Serif</option></select>
                </div>
                <div class="form-group"><label class="text-xs">${NMR.__t('font_size')} (<span id="fontSizeVal">18</span>px)</label>
          ...
          <div class="modal-footer"><button class="btn btn-sm btn-primary px-5" id="saveAllSettingsBtn">${NMR.__t('save_and_apply')}</button></div>
        </div>
      </div>
      <div id="userSettingsModal" class="modal-overlay">
        <div class="modal card">
          <div class="modal-header"><h3>⚙️ ${NMR.__t('profile_settings')}</h3><button class="modal-close" onclick="closeModal()">&times;</button></div>
          <div class="modal-body">
            <form id="userSettingsForm" enctype="multipart/form-data">
              <div class="form-group mb-3">
                <label class="form-label">${NMR.__t('biography')}</label>
                <textarea name="bio" class="form-item" rows="3" maxlength="1000" placeholder="${NMR.__t('tell_us_about_yourself')}"></textarea>
              </div>
              <div class="form-group mb-3">
                <label class="form-label">${NMR.__t('profile_image')}</label>
                <input type="file" name="profile_image" class="form-item" accept="image/*">
              </div>
              <div class="form-group mb-3">
                <label class="form-label">${NMR.__t('cover_image')}</label>
                <input type="file" name="cover_image" class="form-item" accept="image/*">
              </div>
              <div class="grid grid-2 gap-2 mb-3">
                <div class="form-group">
                  <label class="form-label">${NMR.__t('theme')}</label>
                ...
                <div class="form-group">
                  <label class="form-label">${NMR.__t('language')}</label>
                  <select name="lang" class="form-item">
                    <option value="tr">Türkçe</option><option value="en">English</option>
                  </select>
                </div>
              </div>
              <button type="submit" class="btn btn-primary w-100 mt-2">${NMR.__t('update_profile')}</button>
            </form>
          </div>
        </div>
      </div>
    `;
    document.body.insertAdjacentHTML('beforeend', allModals);
  };

/**
 * Melt.js - Lightweight jQuery-like helper library
...
const Melt = (function () {
  ...
  const $ = function (selector) {
    ...
    if (typeof selector === "string") {
      if (selector.trim().startsWith("<")) {
        ...
        this.elements = Array.from(temp.childNodes).filter(node => node.nodeType === 1);
      } else {
  ...
  $.fn = $.prototype = {
    ...
    /* EVENT SYSTEM */
    on(event, selector, callback, options = false) {
      if (typeof selector === "function") {
        return this.each((i, el) => el.addEventListener(event, selector, options));
      }
    ...
    addClass(c) {
      ...
      const classes = c.split(" ").filter(Boolean);
    ...
    closest(selector) {
      return $(this.elements.map(e => e.closest(selector)).filter(Boolean));
    },
  ...
  /* ──────────────────────────────────────────────────────────────
     AJAX HELPERS (fixed GET query + JSON fallback)
  ────────────────────────────────────────────────────────────── */
  $.ajax = function (opts) {

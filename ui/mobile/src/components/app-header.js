 ...
 */
export function renderAppHeader({ title, subtitle = '', back = false, large = false } = {}) {
  return `
    <div class="navbar ${large ? 'navbar-large' : ''}">
      <div class="navbar-bg"></div>
      <div class="navbar-inner sliding">
        <div class="left">
          ${back ? '<a class="link back"><i class="icon f7-icons">chevron_left</i></a>' : ''}
        </div>
        ...
        ${large ? `
          <div class="title-large">
            ...
            ${subtitle ? `<div class="mobile-page-subtitle">${escapeHtml(subtitle)}</div>` : ''}
          </div>
        ` : ''}
      </div>
    </div>

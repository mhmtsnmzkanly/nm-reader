...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  const displayTitle = title || t('feedback.emptyDefaultTitle');
  const displayDesc = description || t('feedback.emptyDefaultMessage');
  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 my-4">
      ...
      <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm mb-6">{displayDesc}</p>
